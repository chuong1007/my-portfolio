...
export default function TagPageHeader({ tagName, resultsCount }: { tagName: string, resultsCount: number }) {
  ...
  return (
    <div className="mb-16">
      <button
        onClick={() => router.back()}
        className="flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors mb-10 group w-fit"
      >
      ...
      <header className="space-y-4">
        <div className="flex items-center gap-4 md:gap-6">
          <div className="flex items-center justify-center w-10 h-10 md:w-16 md:h-16 rounded-full border-2 border-white/20 text-[var(--text-primary)] font-bold text-xl md:text-3xl shrink-0">
            #
          </div>
        ...
        <p className="text-[var(--text-muted)] text-lg md:text-xl max-w-2xl font-medium mt-4">
          {resultsCount > 0 
