...
const newRecoveryCode = `
  useEffect(() => {
    if (!isEditor) return;
    const hasCorrectTitle = expandedBlocks.some(b => b.content.includes("Giới thiệu bản thân"));
    if (!hasCorrectTitle) {
...
console.log('Force fix code injected into About.tsx');
